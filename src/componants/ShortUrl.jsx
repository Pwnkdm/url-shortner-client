...
const { Text } = Typography;
  ...
  return (
    <div style={{height:"80svh",padding:10,background:'#dddddd'}}>
        <div style={{display:"flex",justifyContent:"center",width:"",margin:"auto"}}>
         <Input style={{maxWidth:'300px'}} placeholder="Input your url here..." value={value} onChange={(e)=>setvalue(e.target.value)}/>
         <Button loading={urlDataLoading} disabled={!value} onClick={handleShortUrl} type='primary' >Short Url</Button>
        </div>
        <div className={styles.gridBox}>
            {urlData && urlData.length > 0 &&(
                ...
                {urlData?.map((el,ind)=>(
                    ...
                    <Text  copyable={{text:`https://odd-cyan-raven-boot.cyclic.app/url/${el?.id}`}}>
                        {el?.id}
                    </Text>
                    </div>
                ))
